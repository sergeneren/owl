...
Viewer::Viewer()
{
  ...
  OWLBuffer texcoordBuffer = mesh.texcoord.empty()
   ? nullptr
  ...
  owlTrianglesSetOMMSubdivisionLevel(trianglesGeom,5);

  // ------------------------------------------------------------------
  // create a 4x4 checkerboard texture
  // ------------------------------------------------------------------
  auto opacityTexture = isoSphereModel->textures[0];
  int32_t width = opacityTexture->resolution.x;
  int32_t height = opacityTexture->resolution.y;
  OWLTexture ommTex = owlTexture2DCreate(context,
	  OWL_TEXEL_FORMAT_RGBA8,
	  width,
	  height,
	  opacityTexture->pixel,
	  OWL_TEXTURE_LINEAR,
	  OWL_TEXTURE_CLAMP);
  owlGeomSetTexture(trianglesGeom,"texture",ommTex);
  owlTrianglesSetOMM(trianglesGeom, ommTex);
