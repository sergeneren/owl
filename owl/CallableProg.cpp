...
namespace owl {
  ...
  // ------------------------------------------------------------------
  // CallableProgType::DeviceData
  // ------------------------------------------------------------------

  /*! constructor, only pass-through to parent class */
  CallableProgType::DeviceData::DeviceData(const DeviceContext::SP &device)
  ...
  CallableProgType::CallableProgType(Context *const context,
                             Module::SP module,
                             const std::string &dcName,
                             const std::string &ccName,
                             size_t varStructSize,
                             const std::vector<OWLVarDecl> &varDecls)
    : SBTObjectType(context,context->callableProgTypes,varStructSize,varDecls),
      module(module),
      DirectCallableName(dcName),
      ContinuationCallableName(ccName)
  {}
  
  /*! pretty-printer, for printf-debugging */
  std::string CallableProgType::toString() const
  {
    return "CallableProgType";
  }

  /*! creates the device-specific data for this group */
  RegisteredObject::DeviceData::SP
  CallableProgType::createOn(const DeviceContext::SP &device) 
  {
  ...
  CallableProg::CallableProg(Context *const context,
                     CallableProgType::SP type) 
    : SBTObject(context,context->callableProgs,type)
  {}
  
  /*! pretty-printer, for printf-debugging */
  std::string CallableProg::toString() const
  {
    return "CallableProg";
  }
  ...
  void CallableProg::writeSBTRecord(uint8_t *const sbtRecord,
