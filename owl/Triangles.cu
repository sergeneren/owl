...
namespace owl {
      ...
      const float displacementScale)
  {
    ...
    vec2f subTriBary0, subTriBary1, subTriBary2;
    ...
    micro2bary( subTriIdx, dmmSubdivisionLevelSubTriangles, subTriBary0, subTriBary1, subTriBary2 );
    ...
    unsigned perBlockSubdivisionLevel = fminf( 3u, dmmSubdivisionLevel );
  ...
  void TrianglesGeom::computeDMM(Texture::SP tex)
  {
    ...
    if(displacementScale != 0.0f && subdivisionLevel > 0)
    {
      ...
      if(texDD)
      {
        // We fill in the following DMM data (directions, displacement values, descriptors) on device
        ...
        for (auto device : context->getDevices()) {
          DeviceData &dd = getDD(device);
          dd.dmmArray.d_displacementValues = d_displacementValues.d_pointer;
          dd.dmmArray.d_displacementDirections = d_displacementDirections.d_pointer;
        }
      }
    }
  }
